...
class ServiceOrchestrator:
    ...
    def __init__(self, service_registry=None, debug=False, use_consul=True):
        ...
        Args:
            service_registry: Dictionary mapping service types and names to endpoints
                             If None, will use dynamic discovery, Consul, or a default registry
            debug: Enable debug logging (default: False)
            use_consul: Whether to use Consul for service discovery (default: True)
        """
        # Initialize service registry with priority: provided > dynamic discovery > Consul > fallback
        ...
        if service_registry:
            ...
            try:
                ...
                discovered_registry = get_service_registry()
                if discovered_registry:
                    self.service_registry = discovered_registry
                    logger.info(f"Using dynamic service registry with {sum(len(svc) for svc in discovered_registry.values())} services")
                else:
                    raise Exception("Dynamic discovery returned empty registry")
            except Exception as e:
                logger.info(f"Dynamic service discovery not available: {e}")
                
                # Try Consul as backup if requested and available
                    ...
                    try:
                        ...
                        if self.consul_discovery.is_consul_available():
                            ...
                            if consul_registry:
                                ...
                                logger.info(f"Using Consul service registry with {sum(len(svc) for svc in consul_registry.values())} services")
                            else:
                                self.service_registry = self._get_fallback_registry()
                                logger.info("Using fallback service registry")
                        else:
                            self.service_registry = self._get_fallback_registry()
                            logger.info("Consul not available, using fallback service registry")
                    except Exception as consul_e:
                        logger.warning(f"Consul discovery also failed: {consul_e}")
                        self.service_registry = self._get_fallback_registry()
                        logger.info("Using fallback service registry")
                else:
                    self.service_registry = self._get_fallback_registry()
        ...
        logger.info(f"Initialized ServiceOrchestrator with {sum(len(svc) for svc in self.service_registry.values())} total services")
    ...
    def _get_default_properties(self):
        """Return default service properties - updated to use the new service name"""
        return {
    ...
    def route_task(self, task, variables=None):
        ...
        try:
            ...
            if service_type in self.service_registry and service_name in self.service_registry[service_type]:
                ...
                logger.info(f"Available services: {list(self.service_registry.keys())}")
                for stype, services in self.service_registry.items():
                    logger.info(f"  {stype}: {list(services.keys())}")
