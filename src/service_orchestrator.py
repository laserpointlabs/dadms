...
based on properties defined in BPMN service tasks. Now includes dynamic service
discovery from service_config.json files.
"""
import json
import logging
import requests
    ...
    def __init__(self, service_registry=None, debug=False, use_consul=True):
        ...
        Args:
            service_registry: Dictionary mapping service types and names to endpoints
                             If None, will use dynamic discovery, then Consul, then fallback
        ...
        # Determine service registry in order of preference:
        # 1. Provided registry
        # 2. Dynamic discovery from service_config.json files (preferred when not in Docker)
        # 3. Consul registry (preferred when in Docker)
        # 4. Fallback registry
        if service_registry:
            ...
            try:
                ...
                # If not in Docker container, prefer dynamic discovery over Consul
                # because dynamic discovery uses localhost addresses while Consul uses container IPs
                if dynamic_registry and (not in_docker or not consul_registry):
                ...
                elif consul_registry and in_docker:
                    logger.info("Using Consul service registry (running in Docker)")
                    self.service_registry = consul_registry
                elif consul_registry:
                    logger.info("Using Consul service registry")
                    self.service_registry = consul_registry
                elif dynamic_registry:
                    logger.info("Using dynamic service discovery as fallback")
                    self.service_registry = dynamic_registry
                else:
        ...
        logger.info(f"Initialized ServiceOrchestrator with registry: {list(self.service_registry.keys())}")
    
    def _get_fallback_registry(self):
        """Return fallback service registry"""
        return {
            "assistant": {
                "openai": {
        ...
        if extension_match:
            ...
            if props_match:
                ...
                result = {
                    ...
                    "service.name": props.get("service.name", self._get_default_service_name()),
    ...
    def _get_default_service_name(self):
        ...
        try:
            ...
            if assistant_service:
                ...
                return service_config.get('name', 'openai')
        except Exception:
            pass
