...
interface AASCarProps {
    isLayoutControlled?: boolean; // When true, the layout controls positioning
}

export default function AASCar({ isLayoutControlled = false }: AASCarProps) {
    ...
    const contextIsDragging = isDragging; // Alias for consistency
    ...
    }, [contextDockedHeight, contextDockedWidth, isHydrated]);

    // Update context when local state changes, but avoid loops
    useEffect(() => {
        if (!isHydrated) return; // Don't update context until hydrated
    ...
    }, [height, width, contextIsDocked, contextDockPosition, isHydrated]); // Removed contextDockedHeight and contextDockedWidth from dependencies
...
<body>
    ...
    if (contextIsDocked) {
        ...
        if (contextDockPosition === 'bottom') {
            ...
            // Right docking - handle layout-controlled vs floating mode
            if (isLayoutControlled) {
                // When layout controlled, render without positioning - layout handles it
                return (
                    <div className="flex flex-col overflow-hidden border-l border-theme-border bg-theme-surface h-full">
                ...
                // Original right docking behavior for floating mode
        ...
        return (
            <div
                ref={positionRef}
                className="fixed z-50 flex flex-col overflow-hidden border border-theme-border bg-theme-surface rounded-lg shadow-2xl"
                style={{
                    left: contextPosition.x + 'px',
                    top: contextPosition.y + 'px',
                    width: width + 'px',
                    height: height + 'px',
                    cursor: contextIsDragging ? 'grabbing' : 'default'
                }}
            >
                {/* Title Bar */}
                <div
                    onMouseDown={handlePositionDragStart}
                    className="h-8 bg-theme-bg-secondary border-b border-theme-border flex items-center justify-between px-3 select-none cursor-move"
                >
                    <div className="flex items-center gap-2 text-xs text-theme-text-secondary">
                        <Icon name="hubot" size="xs" />
