...
<bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL"
    ...
    useEffect(() => {
        const initModeler = async () => {
            try {
                ...
                // Note: Main event listeners are set up in the useEffect below
                // to avoid duplicate listeners and ensure proper cleanup

                // Initial XML update to populate the panel
                setTimeout(() => {
                    modelerInstance.saveXML({ format: true }).then((result: any) => {
                        // Simple injection since we just initialized
                        const injectedXml = result.xml;
                        localStorage.setItem('bpmn_model_cache', injectedXml);
                        localStorage.setItem('bpmn_model_timestamp', Date.now().toString());

                        // If XML panel is open, populate it
                        if (xmlPanelVisible && xmlEditorRef.current) {
                            xmlEditorRef.current.value = injectedXml;
                        }
                        console.log('Initial XML updated');
                    });
                }, 50);
    ...
    const clearModel = useCallback(() => {
        if (window.confirm('Are you sure you want to clear the entire model? This cannot be undone.')) {
            if (modeler) {
                console.log('Clearing model and re-initializing...');

                try {
                    // Method 1: Try to clear the canvas completely first
                    const elementRegistry = modeler.get('elementRegistry');
                    const canvas = modeler.get('canvas');

                    // Get all elements and remove them except the process root
                    const allElements = elementRegistry.getAll();
                    console.log('Found elements to clear:', allElements.length);

                    // Clear the canvas completely
                    canvas.removeMarker(canvas.getRootElement(), 'highlight');

                    // Method 2: Import completely fresh empty XML
                    modeler.importXML(emptyXML).then(() => {
                        console.log('Empty XML imported successfully');
                        ...
                        // Force a complete canvas refresh and refit
                        setTimeout(() => {
                            const canvas = modeler.get('canvas');
                            const elementRegistry = modeler.get('elementRegistry');

                            // Verify the canvas is actually clear
                            const remainingElements = elementRegistry.getAll();
                            console.log('Elements after clearing:', remainingElements.length);
                            console.log('Remaining elements:', remainingElements.map((e: any) => ({ id: e.id, type: e.type })));

                            // Reset zoom and view
                            canvas.zoom('fit-viewport');

                            // Force a window resize to ensure proper canvas sizing
                            window.dispatchEvent(new Event('resize'));

                            // Update XML immediately to reflect the cleared state
                            modeler.saveXML({ format: true }).then((result: any) => {
                                localStorage.setItem('bpmn_model_cache', result.xml);
                                localStorage.setItem('bpmn_model_timestamp', Date.now().toString());
                                ...
                                console.log('Model cleared and XML updated');
                                console.log('Final XML preview:', result.xml.substring(0, 300) + '...');
                            }).catch((xmlError: any) => {
                                console.error('Error updating XML after clear:', xmlError);
                            });
                        }, 150); // Slightly longer timeout to ensure import is complete

                    }).catch((error: any) => {
                        console.error('Error importing empty XML:', error);
                        // Fallback: try to clear canvas directly
                        try {
                            const canvas = modeler.get('canvas');
                            const elementRegistry = modeler.get('elementRegistry');

                            // Get all non-root elements and try to remove them
                            const elements = elementRegistry.getAll().filter((element: any) =>
                                element.type !== 'bpmn:Process' && element.id !== 'Process_1'
                            );

                            console.log('Fallback: Removing elements manually:', elements.length);
                            elements.forEach((element: any) => {
                                try {
                                    elementRegistry.remove(element);
                                } catch (removeError) {
                                    console.warn('Could not remove element:', element.id, removeError);
                                }
                            });
                            ...
                            console.log('Using last resort: page reload');
                            window.location.reload();
                        }
                    });

                } catch (error) {
                    console.error('Error during model clearing:', error);
                    // Fallback to page reload if everything fails
                    window.location.reload();
                }
            }
        }
    }, [modeler, emptyXML, xmlPanelVisible]);
    ...
    const updateBasicProperty = useCallback((propertyName: string, value: string) => {
        if (!selectedElement || !modeler) {
            console.warn('Cannot update property: missing selectedElement or modeler');
            return;
        }
        ...
        try {
            ...
            console.log('Updating property:', propertyName, 'to:', value, 'for element:', elementId);
            ...
            // Re-fetch the updated element and update state
            setTimeout(() => {
                try {
                    const updatedElement = elementRegistry.get(elementId);
                    if (updatedElement) {
                        setSelectedElement(updatedElement);
                        console.log('Element updated successfully:', elementId);
                    }
                } catch (error) {
                    console.error('Error re-fetching updated element:', error);
                }
            }, 50); // Small delay to ensure the modeling operation completes
    ...
    }, [xmlPanelVisible, modeler, injectExtensionProperties, emptyXML]);
    ...
    }, [modeler, xmlPanelVisible, selectedElement, propertyCache, diagramProperties, injectExtensionProperties]);
    ...
    // Main event listener setup for comprehensive XML/UI sync
    useEffect(() => {
        if (!modeler) {
            console.log('❌ No modeler available for event listeners');
            return;
        }

        console.log('✅ Setting up event listeners for modeler');

        const updateXmlAndCache = () => {
            console.log('=== XML UPDATE TRIGGERED ===');
            console.log('Modeler available:', !!modeler);
            console.log('XML panel visible:', xmlPanelVisible);
            console.log('XML editor ref:', !!xmlEditorRef.current);

            modeler.saveXML({ format: true }).then((result: any) => {
                console.log('Raw XML from modeler:', result.xml.length, 'characters');
                console.log('XML preview:', result.xml.substring(0, 300) + '...');

                const injectedXml = injectExtensionProperties(result.xml);
                console.log('Injected XML:', injectedXml.length, 'characters');

                localStorage.setItem('bpmn_model_cache', injectedXml);
                localStorage.setItem('bpmn_model_timestamp', Date.now().toString());

                // Always update XML panel if it's visible - this is critical for real-time sync
                if (xmlPanelVisible && xmlEditorRef.current) {
                    xmlEditorRef.current.value = injectedXml;
                    console.log('✅ XML panel updated with new content');
                } else {
                    console.log('❌ XML panel not updated - xmlPanelVisible:', xmlPanelVisible, 'xmlEditorRef.current:', !!xmlEditorRef.current);
                }
            }).catch((error: any) => {
                console.error('❌ Error updating XML and cache:', error);
            });
        };

        // Critical events for XML/UI sync - based on the comprehensive HTML solution
        // The most important events are commandStack.changed and import.done
        const coreXmlUpdateEvents = [
            'commandStack.changed',  // This fires after any change to the model
            'import.done'           // This fires after importing XML
        ];

        // Additional events that might be needed for complete coverage
        const supplementaryEvents = [
        ...
        // Test ALL possible BPMN.js events to see which ones actually fire during operations
        const debugEventHandlers = {
            'element.changed': () => console.log('🔥 element.changed'),
            'commandStack.changed': () => console.log('🔥 commandStack.changed - CRITICAL'),
            'elements.changed': () => console.log('🔥 elements.changed'),
            'shape.added': () => console.log('🔥 shape.added'),
            'shape.removed': () => console.log('🔥 shape.removed'),
            'connection.added': () => console.log('🔥 connection.added'),
            'connection.removed': () => console.log('🔥 connection.removed'),
            'create.start': () => console.log('🔥 create.start'),
            'create.move': () => console.log('🔥 create.move'),
            'create.end': () => console.log('🔥 create.end - POTENTIAL TRIGGER'),
            'create.cleanup': () => console.log('🔥 create.cleanup'),
            'commandStack.preExecute': () => console.log('🔥 commandStack.preExecute'),
            'commandStack.execute': () => console.log('🔥 commandStack.execute'),
            'commandStack.postExecute': () => console.log('🔥 commandStack.postExecute'),
            'commandStack.postExecuted': () => console.log('🔥 commandStack.postExecuted - POTENTIAL TRIGGER'),
            'import.done': () => console.log('🔥 import.done - CRITICAL')
        };

        // Set up debug event listeners to understand what's happening
        Object.entries(debugEventHandlers).forEach(([event, handler]) => {
            modeler.on(event, handler);
        });

        // Set up XML update for core events that are absolutely critical
        coreXmlUpdateEvents.forEach((event: string) => {
            modeler.on(event, updateXmlAndCache);
        });

        // Also set up XML update for supplementary events for complete coverage
        supplementaryEvents.forEach((event: string) => {
            modeler.on(event, updateXmlAndCache);
        });

        console.log('✅ All event listeners attached for comprehensive XML sync');
        console.log('Core XML update events:', coreXmlUpdateEvents);
        console.log('Supplementary events:', supplementaryEvents);

        return () => {
            console.log('🧹 Cleaning up all event listeners');
            Object.entries(debugEventHandlers).forEach(([event, handler]) => {
                modeler.off(event, handler);
            });
            coreXmlUpdateEvents.forEach((event: string) => {
                modeler.off(event, updateXmlAndCache);
            });
            supplementaryEvents.forEach((event: string) => {
                modeler.off(event, updateXmlAndCache);
            });
        };
    }, [modeler, injectExtensionProperties, xmlPanelVisible]);

    // Ensure selection and property panel always reflect the latest BPMN model
    useEffect(() => {
        if (!modeler) return;

        const handleElementClick = (event: any) => {
            try {
                console.log('Element click event:', event.element?.id, event.element?.type);
                if (event.element) {
                    setSelectedElement(event.element);
                }
            } catch (error) {
        ...
        const handleSelectionChanged = (event: any) => {
            try {
                console.log('Selection changed event:', event.newSelection?.length);
                if (event.newSelection && event.newSelection.length > 0) {
    ...
    // Manual XML update function for debugging
    const manualUpdateXml = useCallback(() => {
        console.log('Manual XML update triggered');
        if (!modeler) {
            console.error('No modeler available for manual XML update');
            return;
        }

        modeler.saveXML({ format: true }).then((result: any) => {
            console.log('Manual XML update - Raw XML length:', result.xml.length);

            try {
                const injectedXml = injectExtensionProperties(result.xml);
                console.log('Manual XML update - Injected XML length:', injectedXml.length);

                // Update cache
                localStorage.setItem('bpmn_model_cache', injectedXml);
                localStorage.setItem('bpmn_model_timestamp', Date.now().toString());

                // Update XML panel if visible
                if (xmlPanelVisible && xmlEditorRef.current) {
                    xmlEditorRef.current.value = injectedXml;
                    console.log('✅ Manual XML panel update successful');
                } else {
                    console.log('❌ Manual XML panel update failed - xmlPanelVisible:', xmlPanelVisible, 'xmlEditorRef.current:', !!xmlEditorRef.current);
                }
            } catch (error) {
                console.error('Error during manual XML injection:', error);
                // Fallback to raw XML
                if (xmlPanelVisible && xmlEditorRef.current) {
                    xmlEditorRef.current.value = result.xml;
                    console.log('✅ Manual XML panel update with raw XML');
                }
            }
        }).catch((error: any) => {
            console.error('Error during manual XML update:', error);
        });
    }, [modeler, injectExtensionProperties, xmlPanelVisible]);
                                    ...
                                    <>
                                        ...
                                        >
                                            <div className="xml-panel-header">
                                                <h4>BPMN XML</h4>
                                                <div className="xml-panel-controls">
                                                    <button
                                                        className="xml-update-btn"
                                                        onClick={manualUpdateXml}
                                                        title="Manually update XML for debugging"
                                                    >
                                                        🔄 Update XML
                                                    </button>
                                                    <button
                    ...
                    <div className="bottom-toolbar">
                        <div className="toolbar-left">
                            ...
                            <button
                                ...
                                onClick={() => {
                                    console.log('🧪 Manual XML update test triggered');
                                    if (modeler) {
                                        modeler.saveXML({ format: true }).then((result: any) => {
                                            console.log('🧪 Manual test - XML length:', result.xml.length);
                                            if (xmlPanelVisible && xmlEditorRef.current) {
                                                xmlEditorRef.current.value = result.xml;
                                                console.log('🧪 Manual test - XML panel updated');
                                            }
                                        });
                                    }
                                }}
                                title="Test XML Update"
                            >
                                🧪 Test XML
                            </button>
                        </div>
