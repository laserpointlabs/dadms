...
function Test-DADMSystem {
    ...
    try {
        ...
        $discoveryScript = "from config.service_registry import discover_services; services = discover_services(); count = sum(len(svc) for svc in services.values()); print(f'Discovered {count} services')"
...
function Show-Help {
    ...
    Write-Host ""
    Write-Host "Usage: .\dadm_manager_clean.ps1 [action]" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "  .\dadm_manager_clean.ps1 status"
    Write-Host "  .\dadm_manager_clean.ps1 restart"
    Write-Host "  .\dadm_manager_clean.ps1 test"
    Write-Host ""
